module Set 
    (Set, emptyS, addS, belongs, removeS, set2list) 
where
...
-- COSTO: O(n) (o O(log n) en promedio).
...
-- COSTO: O(n^2) o O(n) según corresponda.
set2list (S t) = inorder t

-- #################################################### AUXILIARES ####################################################

buscarBST :: Ord a => a -> Tree a -> Bool  
-- COSTO: O(n), en peor caso, pero es O(log n) en promedio.
-- PRECOND: el árbol es BST.
buscarBST x EmptyT          = False
buscarBST x (NodeT y ti td) = if (x == y) then True else 
                              if (x < y)  then buscarBST x ti
                                          else buscarBST x td
...
insertarBST x (NodeT y ti td) = if (x == y) then NodeT x ti td else 
                                if (x < y)  then NodeT y (insertarBST x ti) td
                                            else NodeT y ti (insertarBST x td)

borrarBST :: Ord a => a -> Tree a -> Tree a
-- PRECOND: el árbol es BST.
borrarBST _ EmptyT          = EmptyT
borrarBST x (NodeT y ti td) = if (x == y) then rearmarBST ti td else 
                              if (x < y)  then NodeT y (borrarBST x ti) td
...
inorder :: Tree a -> [a]   
-- COSTO: O(n^2), si (++) es O(n), pero O(n), si (++) es O(1).
inorder EmptyT          = []
