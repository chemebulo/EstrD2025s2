...
// COSTO TIEMPO: O(n).
    // Siendo de costo lineal, ya que por cada elemento del árbol se utilizan operaciones de costo constante.
// COSTO ESPACIO: O(n).
    // Siendo de costo lineal, ya que por cada elemento del árbol se utiliza un frame en la memoria stack.
    if(isEmptyT(t)){
        return 0;
    } else {
        return  sumarT(t->left) + t->elem + sumarT(t->right);
    }
}
...
// COSTO ESPACIO: O(n).
    ...
    if(isEmptyT(t)){
        ...
        return sumarT(t->left) + 1 + sumarT(t->right);
    }
}
...
// COSTO ESPACIO: O(n).
    ...
    if(isEmptyT(t)){
        ...
        return (perteneceT(e, t->left) || (rootT(t) == e) || perteneceT(e, t->right));
    }
}
...
// COSTO ESPACIO: O(n).
    ...
    if (isEmptyT(t)){
        ...
        if(rootT(t) == e){
            return aparicionesT(e, left(t)) + 1 + aparicionesT(e, right(t));
        } else {
